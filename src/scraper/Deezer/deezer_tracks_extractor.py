...
from src.config import DEEZER_TRACKLIST_ROW_SELECTOR, DEEZER_INFORMATIONS_SELECTOR, DEEZER_TITLE_SELECTOR, DEEZER_ALBUM_ARTIST_NAME_SELECTOR, DEEZER_TRACKLIST_ROW_MUSIC_SELECTOR, DEEZER_TRACKLIST_ROW_MUSIC_SELECTOR, DEEZER_PLAYLIST_ALBUM_SELECTOR, DEEZER_TRACKLIST_ROW_ARTIST_SELECTOR
...
def deezer_extract_album_tracks(driver, tracks, _):
  ...
  # The artist cell in the music row may be empty
  driver.implicitly_wait(0) 
  
  for page_track in page_tracks:
    try:
      # Deezer doesn't include the album artist in the music row
      artists = [album_artist_name] # Add album artist name on a empty array

      first_div = page_track.find_element(By.TAG_NAME, "div")
    
      # Get and clean music name : 1. Delilah (pull me out of this) -> Delilah (pull me out of this)
      # ^    → Start of the string  
      ...
      music_name = first_div.find_element(By.CSS_SELECTOR, DEEZER_TRACKLIST_ROW_MUSIC_SELECTOR)
      music_name_cleaned = re.sub(r"^\d+\.\s*", "", music_name.text)
...
def deezer_extract_playlist_tracks(driver, tracks, _):
  ...
  for page_track in page_tracks:
    try:
      ...
      if not (album and artists and music_name):
        continue  
